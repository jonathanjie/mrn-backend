...
from enum import NoonReportTypes, ReportTypes, ShipAccessPrivilege, ShipTypes, Status
...
class Ship(models.Model):
    ...
    company             = models.ForeignKey(Company, on_delete=models.PROTECT)
    ship_type           = models.ChoiceField(choices=ShipTypes.choices)
...
class ShipUser(models.Model):
    ...
    privilege           = models.ChoiceField(choices=ShipAccessPrivilege.choices)
    status              = models.IntegerField(choices=Status.choices)
    date_created        = models.DateTimeField(auto_now_add=True)
    date_modified       = models.DateTimeField(auto_now=True)
...
class NoonReportHeader(models.model):
    report_header       = models.OneToOneField(ReportHeader, on_delete=models.PROTECT, primary_key=True)
    noon_report_type    = models.ChoiceField(choices=NoonReportTypes.choices)
    date_created        = models.DateTimeField(auto_now_add=True)
    date_modified       = models.DateTimeField(auto_now=True)

class NoonReportAtSea(models.model):
    noon_report_header  = models.OneToOneField(NoonReportHeader, on_delete=models.PROTECT, primary_key=True)
